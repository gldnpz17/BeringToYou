  ...
  return (
    <StyledContainer className='p-0 mt-5 pt-3'>
      ...
      <div className='mx-2 mb-2 d-flex flex-column'>
        <SearchTextBox placeholder='Cari Produk' className='mb-2' />
      ...
      <div className='d-flex flex-row justify-content-end mx-2 mb-1'>
        <IconButton className='ms-2 p-1' iconOnly={true} onClick={() => setDisplayMode('list')}>
          <ViewListIcon style={{width: '1.6rem', height: '1.6rem'}} />
        </IconButton>
        <IconButton className='ms-2 p-1' iconOnly={true} onClick={() => setDisplayMode('card')}>
          <ViewModulesIcon style={{width: '1.6rem', height: '1.6rem'}}/>
        </IconButton>
      </div>

      <Row className={`${(displayMode === 'list' ? 'row-cols-1' : 'row-cols-2')} mx-2`}>
